...
/**
 * Sequalise Database Adapter
 * @classdesc Adapter for Sequalise operations using Sequalise
 * @constructor
 * @param {Object} model - Sequalise model
 * @param {Object} options - Options for the Sequalise adapter
 */
function SequaliseCrudder(model, options) {
    this.model = model;
    ...
    if (!this.model) {
        throw new Error('Sequalise model is required for Sequalise adapter');
    }
}

SequaliseCrudder.prototype._logError = function (req, err, message = null) {
    if (this.logger) {
        this.logger.error(`${req.headers[this.options.trackingId] || req.headers.host} - ${message || err.message}`, err);
    }
};

SequaliseCrudder.prototype._customizer = function (objValue, srcValue) {
    if (_.isArray(objValue)) {
        return srcValue;
    }
};

SequaliseCrudder.prototype.count = async function (req, res) {
    try {
...
SequaliseCrudder.prototype.find = async function (req, res) {
    try {
        let params = parseParams(req);
        let filter = parseSQLFilter.FilterParse(params.filter);
...
SequaliseCrudder.prototype.findById = async function (req, res) {
    try {
        let params = parseParams(req);
        let filter = {
...
SequaliseCrudder.prototype.create = async function (req, res) {
    try {
        let params = parseParams(req);
...
SequaliseCrudder.prototype.updateById = async function (req, res) {
    try {
        let params = parseParams(req);
        let id = params.id;
        let data = params.data;
...
SequaliseCrudder.prototype.updateByFilter = async function (req, res) {
    try {
        let params = parseParams(req);
        let filter = parseSQLFilter.FilterParse(params.filter);
        let data = params.data;
...
SequaliseCrudder.prototype.deleteById = async function (req, res) {
    try {
        let params = parseParams(req);
        let id = params.id;
        let filter = {
...
SequaliseCrudder.prototype.deleteByFilter = async function (req, res) {
    try {
        let params = parseParams(req);
        let filter = parseSQLFilter.FilterParse(params.filter);
        let documents = await this.model.find(filter).exec();
...
SequaliseCrudder.prototype.bulkShowByIds = async function (req, res) {
    try {
        let params = parseParams(req);
        let ids = params.ids;
...
SequaliseCrudder.prototype.bulkUpdateByIds = async function (req, res) {
    try {
        let params = parseParams(req);
        let ids = params.ids;
        let filter = {
...
SequaliseCrudder.prototype.bulkDeleteByIds = async function (req, res) {
    try {
        let params = parseParams(req);
        let ids = params.ids;
        let filter = {
...
module.exports = SequaliseCrudder;
